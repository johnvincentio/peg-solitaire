...
* Status: droppable = true for tiles that can be dropped, and from a tile allowing a drag.
*/
		setStatus: function() {
			for (var x = 0; x < 7; x++) {
				var row = this.rows[x];
				for (var y = 0; y < 7; y++) {
					...
					if (this.isOccupied(x, y)) {
						...
						if (this.anyLegalFromMoves(x, y)) {
							...
							item.left_move = this.isFromLeftMoveLegal(x, y);
							item.right_move = this.isFromRightMoveLegal(x, y);
						} else {
...
*/
		anyLegalFromMoves: function(row, column) {
			return (
				this.isFromUpMoveLegal(row, column) ||
...
*/
		anyLegalToMoves: function(row, column) {
			return (
				this.isToUpMoveLegal(row, column) ||
